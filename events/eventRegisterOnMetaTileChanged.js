export const id = "EVENT_REGISTER_ON_META_TILES_CHANGED";
export const name = "Register on actor metatile changed";
export const groups = ["Meta Tiles"];

export const autoLabel = (fetchArg) => {
  return `Register on actor metatile changed`;
};
...
export const compile = (input, helpers) => {
  ...
  const symbol = scene.symbol + "_on_metatile_changed";
  //load to scene
	_stackPushConst(`_${symbol}`);
	_stackPushConst(`___bank_${symbol}`);  		
	_callNative("load_metatile_changed_events");
	_stackPop(2);
